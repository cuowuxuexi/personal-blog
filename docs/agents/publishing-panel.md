...
- The WeChat preview is a job-scoped local artifact outside the Git manifest. It uses snapshot-local images for preview. Copy embeds those images as JPEG in the clipboard after production SHA verification or a successful online-asset check, so the Official Account editor does not have to transload WebP. Markdown remains the only content source.
