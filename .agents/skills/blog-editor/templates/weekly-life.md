...
4. 无外链就删 `link-href`；无副标题就删 `subtitle`；无主图就删 `image`；无徽标就删 `badge-image` / `badge-alt`。有 `subtitle` 时，`link-href` 挂在副标题上，主标题保持纯文本。
5. `issue` = 同分类最高期数 + 1；`date` 默认当天；文件名默认 `YYYY-MM-DD.md`。
